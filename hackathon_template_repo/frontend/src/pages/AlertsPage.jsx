...
const ALERTS = [
  {
    ...
    indicators: ['New device fingerprint', 'Geo-velocity anomaly', 'Fresh beneficiary', 'High transfer velocity'],
    original:
  ...
  },
  {
    ...
    indicators: ['Merchant cluster risk', 'Retry after decline', 'Night-time spike', 'Unusual MCC'],
    original:
  ...
  },
  {
    ...
    indicators: ['Thin credit file', 'Document reuse signal', 'Employer mismatch', 'Shared contact metadata'],
    original:
  ...
  },
  {
    ...
    indicators: ['PIN reset proximity', 'Above baseline cashout', 'Known city', 'No failed auth'],
    original:
...
const AlertsPage = () => {
  ...
  const filteredAlerts = useMemo(() => {
    ...
    return ALERTS
      ...
      .filter((alert) => {
        ...
        return [alert.id, alert.customer, alert.account, alert.type, alert.channel]
          .join(' ')
          .toLowerCase()
  ...
  const selectedAlert = filteredAlerts.find((alert) => alert.id === selectedId) || filteredAlerts[0] || ALERTS[0];
  ...
  const handleUpload = async (event) => {
    ...
    try {
      ...
      setUploadError(err.response?.data?.error || err.message || 'Alert upload failed.');
    } finally {
      setUploading(false);
    }
  };
      ...
      <div className="alerts-wrapper">
        <div className="alerts-header">
          <div>
            ...
            <p className="alerts-subtitle">AI-generated alert summaries, risk indicators, severity ranking, and analyst feedback.</p>
          </div>
        ...
        <div className="insight-grid">
          <div className="kpi-grid">
            ...
            <div className="kpi-card" data-tone="primary">
              <div className="kpi-icon">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" d="M4 19V5m0 14h16M8 17v-6m4 6V7m4 10v-3" />
                </svg>
              </div>
              <div>
            ...
            <div className="kpi-card" data-tone="neutral">
              <div className="kpi-icon">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" d="M13 7h8m0 0v8m0-8-8 8-4-4-6 6" />
                </svg>
              </div>
              <div>
          ...
          <div className="chart-panel">
            ...
            <div>
              ...
              <p className="panel-subtitle">Current queue distribution by alert label.</p>
              <div className="legend-list" style={{ marginTop: '14px' }}>
                <div className="legend-row">
                  <span className="legend-label"><span className="legend-dot critical" />Critical</span>
                  <span>{severityCounts.Critical}</span>
                </div>
                <div className="legend-row">
                  <span className="legend-label"><span className="legend-dot high" />High</span>
                  <span>{severityCounts.High}</span>
                </div>
                <div className="legend-row">
                  <span className="legend-label"><span className="legend-dot medium" />Medium</span>
                  <span>{severityCounts.Medium}</span>
                </div>
                <div className="legend-row">
                  <span className="legend-label"><span className="legend-dot low" />Low</span>
        ...
        <form className="upload-panel" onSubmit={handleUpload}>
          <div>
            ...
            <p className="panel-subtitle">Submit generated fraud alert input data to backend processing.</p>
            <div className="upload-fields" style={{ marginTop: '14px' }}>
              <div className="field">
                <label className="field-label">File Type</label>
                <select className="select" value={fileType} onChange={(event) => setFileType(event.target.value)}>
              ...
              <div className="field">
                ...
                <input
                  ...
                  type="file"
                  accept={fileType === 'json' ? '.json,application/json' : '.xml,text/xml,application/xml'}
                  onChange={(event) => {
            ...
            {uploadResult && (
              <div className="upload-message success">
                {uploadResult.message}: {uploadResult.record_count} records queued from {uploadResult.file_name}
              </div>
            )}
          </div>
        ...
        <div className="alerts-layout">
          <div className="alert-panel">
            <div className="panel-header">
              ...
              <p className="panel-subtitle">Sorted worklist for rapid analyst triage.</p>
            </div>
            <div className="alert-list">
              {filteredAlerts.map((alert) => (
                ...
                >
                  <div className="severity-ring" data-tone={getTone(alert.severity)}>
                    {alert.severity}
                  </div>
                  <div className="alert-main">
                    <h3 className="alert-title">{alert.id} / {alert.type}</h3>
          ...
          <div className="detail-panel">
            <div className="panel-header">
              ...
              <p className="panel-subtitle">{selectedAlert.id} case intelligence and original alert trace.</p>
            </div>
            <div className="detail-body">
              <div className="badge-row">
                <span className="badge">Severity {selectedAlert.severity}</span>
                <span className="badge">{selectedAlert.status}</span>
                <span className="badge">{formatCurrency(selectedAlert.amount)}</span>
              </div>
              ...
              <div className="detail-block">
                ...
                <div className="badge-row">
                  {selectedAlert.indicators.map((indicator) => (
                    <span className="badge" key={indicator}>{indicator}</span>
                  ))}
                </div>
              </div>
              ...
              <div className="action-row">
                <a className="btn-primary" href={selectedAlert.caseUrl}>Open Case Tool</a>
                <button className="btn-secondary" type="button">Escalate</button>
                <button className="btn-secondary" type="button">Mark Reviewed</button>
              </div>
              ...
              <div className="feedback-box">
                ...
                <div className="action-row">
                  <button
                    ...
                    type="button"
                    onClick={() => setFeedback((prev) => ({ ...prev, [selectedAlert.id]: 'useful' }))}
                  >
                    Useful
                  </button>
                  <button
                    ...
                    type="button"
                    onClick={() => setFeedback((prev) => ({ ...prev, [selectedAlert.id]: 'needs_review' }))}
                  >
                    Needs Review
                  </button>
                </div>
        ...
        <div className="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="alert-modal-title">
          <div className="modal-panel">
            <div className="modal-header">
              <div>
                <h2 className="panel-title" id="alert-modal-title">{modalAlert.id} Alert Properties</h2>
                <p className="panel-subtitle">Complete alert context and model-generated triage details.</p>
              </div>
              <button className="btn-icon" type="button" aria-label="Close alert details" onClick={() => setModalAlert(null)}>
                x
              </button>
            </div>
            ...
            <div className="modal-body">
              <div className="property-grid">
                ...
                <div className="property-item">
                  ...
                  <span className="property-value">{formatCurrency(modalAlert.amount)}</span>
                </div>
                <div className="property-item">
                ...
                <div className="property-item">
                  ...
                  <span className="property-value">{modalAlert.original.split(';')[0]}</span>
                </div>
                <div className="property-item">
              ...
              <div className="detail-block">
                ...
                <p>{modalAlert.status === 'Critical' ? 'Pending urgent review' : 'Pending analyst validation'}</p>
              </div>
            </div>
